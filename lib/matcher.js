...
Matcher.prototype.matchAsync = async function(path, value, index) {
  if (arguments.length === 1) {
    value = path;
    path = '';
  }
  ...
  if (!this._matchAsync) {
    ...
    return Promise.resolve(this.match(path, value, index));
  }

  const errors = await this._matchAsync(path, value, index)
  if (!errors) { return []; }
